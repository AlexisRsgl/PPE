    ...
    <div class="container">
        <nav class="side-nav">
            <ul class="nav-menu">
                ...
                @endif
                @if($role == '1' ||$role ==  '5')
                    <li class="nav-item active"><a href="./vehicule"><i class="fas fa-sign-out-alt"></i><span class="menu-text">Gestion des véhicules du fournisseur</span></a></li>
                @endif
                @if($role == '1' ||$role ==  '6')
    ...
    <section class="section_table">
        ...
        <table>
            ...
            @foreach ($tableauDeVoiture as $car)
                @if(isset($car->mark) && $car->vendor_id != null)
                    <tr>
                        <td>{{ $car->mark }} </td>
                        <td>{{ $car->model }}</td>
                        ...
                        @foreach($tableauAgences as $agence)
                            @if($agence->id == $car->agency_id)
                                <td>{{ $agence->name }}</td>
                                <td> {{ $agence->location }} </td>
                            @endif
                        @endforeach
                        @if($car->agency_id == null)
                        <td></td>
                        <td></td>
                        @endif
...
<div class="login-popup">
    <div class="form-popup" id="popupForm">
        <form action="{{ route("creerVehiculeFournisseur") }}" class="form-container">
            <h2>Ajoutez un véhicule</h2>
            <label for="marque">
                <strong>Marque</strong>
            </label>
            <input type="text" id="marque" placeholder="Marque" name="marque" required />
            <label for="model">
                <strong>Modèle</strong>
            </label>
            <input type="text" id="model" placeholder="Modèle" name="model" required />
            <label for="immatriculation">
                <strong>Immatriculation</strong>
            </label>
            <input type="text" id="immatriculation" placeholder="Immatriculation du véhicule" name="immatriculation" required />
...
<div class="login-popup">
    <div class="form-popup" id="popupModifForm">
        <form action="{{ route("modifierVehiculeFournisseur") }}" class="form-container">
            ...
            <input id="vehiculeIdToAffect" name="vehiculeIdToAffect" type="hidden" value="">
...
<script>
    ...
    function openModifForm(vehiculeId) {

        window.localStorage.setItem("vehiculeToAffect",vehiculeId);
        if (localStorage.getItem("vehiculeToAffect") != null) {
            document.getElementById("vehiculeIdToAffect").value = window.localStorage.getItem("vehiculeToAffect");
        }
        document.getElementById("popupModifForm").style.display = "block";
    }

    function closeModifForm() {
        document.getElementById("popupModifForm").style.display = "none";
    }
